...
# سقف بكسلات الصفحة المرسومة لفحص الحبر. مواصفة PDF تسمح بصفحة حتى
# ١٤٤٠٠×١٤٤٠٠ نقطة (٢٠٠ بوصة)، وهي عند ١٥٠ نقطة/بوصة تعطي ٩٠٠ مليون بكسل
# ومنها نسخة numpy ثانية — قِيس فعليًا: ٢٫٦ غيغابايت و١٢ ثانية من ملف حجمه
# ٨٠٠ بايت. الصفحة الأكبر من السقف تُرسم بدقة أدنى: فحص حبر أقل دقة عليها
# وحدها مقايضة مقبولة مقابل ألّا تموت العملية بنفاد الذاكرة.
#
# السقف هدف لا حدّ حسابي دقيق: get_pixmap يجبر الأبعاد إلى بكسلات صحيحة
# بالتقريب لأعلى، فقد يتجاوز الناتج السقف بأقل من صفٍّ وعمود — أي بأجزاء
# من الألف. الغرض منع تخصيص بمئات الأضعاف، لا محاسبة بكسل ببكسل.
INK_MAX_PIXELS = 40_000_000


def safe_zoom(page, dpi, max_pixels):
    """
    معامل التكبير الذي يرسم الصفحة بالدقة المطلوبة **دون** تجاوز سقف البكسلات.

    كل من يرسم صفحة غير موثوقة يحتاج هذا: مواصفة PDF تسمح بصفحة ١٤٤٠٠ نقطة،
    ورسمها بالمعامل الاسمي يطلب مليارات البكسلات من ملف حجمه أقل من كيلوبايت.
    والدالّة عامة لا خاصة لأن مسارين يرسمان الصفحة — فحص الحبر والـOCR —
    وكان السقف في الأول وحده، فبقي الثاني مكشوفًا. حساب السقف مرّتين يعني
    نسختين تتباعدان، وقد تباعدتا فعلًا قبل توحيدهما هنا.

    يرجّع المعامل كما هو إن كانت الصفحة تحت السقف — فالتنزيل لا يمسّ إلا
    الصفحة العملاقة وحدها.
    """
    z = dpi / 72.0
    area = abs(page.rect.width * z) * abs(page.rect.height * z)
    if area > max_pixels and area > 0:
        z *= (max_pixels / area) ** 0.5
    return z
...
WM_ALPHA = 0.60
# لمعان اللون أعلى من هذا = نص رمادي باهت. أختام «صورة طبق الأصل» وشعارات
# الجهات تُرسم في الغالب بلون رمادي **معتم** لا بشفافية، فلا يمسكها WM_ALPHA.
# القياس: المتن الأسود ٠٫٠٠، والرمادي #CCC ٠٫٨٠، والرمادي المتوسط #808080
# ٠٫٥٠ — فالعتبة تفصل الختم الباهت عن أي نص لا يزال مقروءًا كمتن.
WM_LUMA = 0.62
...
def _faint(span):
    """
    هل الجزء باهت؟ بإشارتين مستقلتين — أي واحدة تكفي.

    الشفافية: alpha في rawdict عدد من ٠ إلى ٢٥٥.

    اللون: أكثر الأختام والشعارات تُرسم رماديًا **معتمًا** (alpha=255) لا
    شفافًا، فالشفافية وحدها كانت تتركها. والأسوأ أنها تنجو كبيرةَ الخط
    فتُصنَّف عنوانًا في classify وتدخل الفهرس المولَّد. الحقل color عدد
    sRGB مرزوم في rawdict، فيُفكّ ويُقاس لمعانه.
    """
...
def ink_map(page, dpi=INK_DPI):
    ...
    الدقة تنزل تلقائيًا على الصفحة العملاقة: بلا هذا السقف كانت صفحة
    ١٤٤٠٠×١٤٤٠٠ نقطة (وهي مشروعة في مواصفة PDF) تطلب ٩٠٠ مليون بكسل،
    فيموت التطبيق بنفاد الذاكرة من ملف حجمه أقل من كيلوبايت. المستدعي
    يستعمل z المُرجَع لا الافتراضي، فالتنزيل شفّاف تمامًا لبقية المسار.
    """
    z = safe_zoom(page, dpi, INK_MAX_PIXELS)
