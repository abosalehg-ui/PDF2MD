...
def verdict_of(rows, ligatures=0):
    """
    يرجّع (نص الحكم، هل النتيجة سليمة) من جدول قبل/بعد للكلمات المؤشّرة.

    `ligatures` عدد الرباطات التي أصلحها المحرّك فعلًا في العيّنة. بدونه كان
    الحكم يقرأ جدول الكلمات المؤشّرة وحده، وهو ثماني كلمات من معجم الأنظمة
    السعودية. المستند الذي لا يذكر «المادة» ولا «اللائحة» — خطاب أو مذكرة أو
    عقد — يخرج جدوله أصفارًا كلَّه، فيُحكم عليه بالسلامة وإن كان كل رباط فيه
    مقلوبًا. والضرر ليس في الرسالة وحدها: المستخدم يقرأ «الملف سليم» فيمرّ
    بـ`--no-ligatures` أو يترك الأداة أصلًا، فيخرج نصّه «هللا» و«السالم».

    فالصمت عن عدد الرباطات هو ما يصنع الحكم الكاذب، لا نقصُ الكلمات المؤشّرة.
    """
    before_bad = sum(r["before_bad"] for r in rows)
    after_bad = sum(r["after_bad"] for r in rows)
    if before_bad == 0:
        if ligatures:
            return (f"لم تظهر الكلمات المؤشّرة في العيّنة، لكن أُصلح "
                    f"{ligatures:,} رباطًا مقلوبًا فيها — الملف يحتاج الإصلاح، "
                    f"فلا تعطّله بـ--no-ligatures."), True
