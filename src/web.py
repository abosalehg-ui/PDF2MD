...
def build_options(raw=None):
    ...
    # لا Tesseract داخل Pyodide — تشغيله يحتاج ثنائيًا لا وجود له — فحساب
    # حكم الـOCR في المتصفّح ضائع بالكامل: يفحص كل صفحة ثلاثة فحوص ليخرج
    # بقرار لا يمكن تنفيذه. التعطيل الصريح هنا أصدق من الاتّكال على أن
    # المحرّك سيكتشف الغياب بنفسه، وأسرع.
    opt.ocr = "never"
