...
بعض المولِّدات ترسم الياء بجليفين: قاعدة مهملة النقط تُصدَّر **مسافةً بعرض حقيقي**، والنقطتان تُصدَّران تنوينًا **بعرض صفر**. الناتج `تف ٌد` بدل `تفيد` و`الش ٌوخ` بدل `الشيوخ`.

القاعدة: مسافة لها عرض حقيقي + تنوين بعرض صفر يبدأ في **النصف الأول من صندوق المسافة** ← ياء واحدة تأخذ صندوق المسافة.

شرط الموضع هو ما يفصلها عن التشكيل الطائر (القسم ٧): `وشكرًا` تُصدَّر أيضًا تنوينًا بعرض صفر بعد مسافة، لكن التنوين هناك يقع عند الحافة **المقابلة** لأنه يخصّ حرفًا في كلمة تالية.
...
| الحالة | ما يخرج بلا OCR | الرصد |
|---|---|---|
| خريطة `ToUnicode` مكسورة كليًا | حروف لاتينية عشوائية: `.Sg0rlg gpl-r o ll` بدل `المحامي والموثق` | أغلب «الكلمات» اللاتينية بلا حرف علة، والعربي فيها أقل من ١٥٪ |
| صفحة ممسوحة ضوئيًا | لا شيء — الصفحة صورة وفوقها ختم فقط | متن الصفحة (بعد حذف العلامة المائية) أقل من ٤٠ حرفًا وصورة تغطيها |
| خريطة `ToUnicode` مبدَّلة جزئيًا | حروف عربية لكن بعضها مُستبدَل: `عيى` بدل `على` | ثلاث ياءات مكسورة فأكثر (القسم ٨) — أثر جانبي يتركه المولِّد نفسه |
...
**Tesseract اختياري.** بدونه تعمل الأداة كما كانت، وتقول لك مرة واحدة في أول التحويل إنه غائب وإن الصفحات الممسوحة أو المعطوبة الخط ستخرج ناقصة — بدل أن تُخرج ملفًا فارغًا بلا إنذار.

**والخسارة تُعلَّم في الناتج نفسه** لا في سجلّ التحويل وحده: الصفحة التي لم يخرج منها سطر واحد وهي تستحق الـOCR تترك مكانها سطرًا ظاهرًا:

```
> ⚠ ص 4: صفحة ممسوحة ضوئيًا بلا طبقة نص — لم تُقرأ، فلا نص لها هنا.
```

بدون العلامة كانت الصفحة تختفي بلا أثر: مذكرة من ست صفحات، ثلاثٌ منها مرفقات ممسوحة، تخرج بنص ثلاث صفحات فقط — وسطر الملخّص يقول «استُخرجت ٦ صفحة»، فيطمئن القارئ إلى ملفٍ ينقصه نصف المستند ولا شيء فيه يدلّه على ما ضاع.
...
ويشترط معه **أن يفتتح العنوان ما بعده**: السطر الذي يسبقه فقرةٌ لم تُختم جملتها هو تكملتها لا عنوانٌ جديد، مهما كبر خطه. بدون هذا الشرط يُقتطع نصفُ جملة عنوانًا — «…أرفع أسمى عبارات الشكر إلى مقام» ثم **«سيدي صاحب السمو…»** بخط أعرض — فتنقطع الجملة ويدخل نصفها الفهرس، بينما تبقى بقيّة الكتلة نفسها فقرةً لأنها تنتهي بفاصلة، فينشطر بلوك بصري واحد عنوانًا وفقرة. والشرط مقصور على ما سبقه **فقرة**، لأن العنوان يتلو العنوان كثيرًا («الباب الأول» ثم «الفصل الأول») وكلاهما بلا نقطة.

ويُستثنى صراحةً ما ليس عنوانًا وإن حقّق كل شرط شكلي:

| المستثنى | لماذا |
|---|---|
| **البسملة** | افتتاحٌ ديني يتصدّر أكثر المستندات العربية الرسمية، يُصفّ عريضًا كبيرًا — ولا قسم تحته ولا معنى لإدراجه في فهرس |
| **ما دون ثلاثة حروف** | سطر الـOCR المشوّش (`© 5ه`) يخرج بخط كبير فيُرقّى عنوانًا ويدخل الفهرس، فيلوّث بنية المستند بضجيج |
...
```bash
sudo apt install tesseract-ocr tesseract-ocr-ara   # Ubuntu/Debian
...
الفحص يعرض جدول «قبل/بعد» لثماني كلمات مؤشّرة من معجم الأنظمة السعودية (`المادة`، `اللائحة`، `المملكة`…). وهي كلمات دلالتها قاطعة حين تظهر، لكن **غيابها لا يدلّ على شيء**: المستند الذي لا يذكرها — خطاب أو مذكرة أو عقد — يخرج جدوله أصفارًا كلَّه وإن كان كل رباط فيه مقلوبًا.

لذلك لا يُبنى الحكم على الجدول وحده، بل على **عدد الرباطات التي أصلحها المحرّك فعلًا** في العيّنة:

| الجدول | الرباطات المُصلَحة | الحكم |
|---|---|---|
| صفر | صفر | الملف سليم — لا حاجة للإصلاح |
| صفر | `> 0` | لم تظهر الكلمات المؤشّرة، لكن الملف يحتاج الإصلاح — فلا تعطّله بـ`--no-ligatures` |
| `> 0` | — | رُصد التلف، ويُقال كم أُصلح منه وكم بقي |

الفرق ليس في الرسالة وحدها: «الملف سليم» تُغري بـ`--no-ligatures` أو بترك الأداة أصلًا، فيخرج النص `هللا` و`السالم` و`األعمال`.
...
# مستند عربي خالص ممسوح ضوئيًا — لغة واحدة أنقى من ara+eng
python main.py مذكرة.pdf --ocr-lang ara -o مذكرة.md

# تعطيل القراءة الضوئية كليًا — أسرع، وناتج الصفحة المعطوبة ناقص
python main.py ملف.pdf --ocr never -o ملف.md
...
| **زر الفحص التشخيصي** | جدول قبل/بعد لثماني كلمات مؤشّرة + عدد الرباطات المُصلَحة + عيّنة نص + هل الملف مصوّر يحتاج OCR |
...
**الترويسات الأمنية:** `frame-ancestors` تتجاهلها المواصفة حين تُسلَّم بوسم `meta`، فسياسة أمن المحتوى في رأس الصفحة لا تمنع تأطيرها في `iframe`. ولذلك يرسل `vercel.json` الترويسات الحقيقية: `frame-ancestors 'none'` و`nosniff` و`no-referrer`، ومعها `COOP`/`COEP` اللتان تفتحان باب `SharedArrayBuffer` — أي إيقافًا يقاطع بايثون بدل قتل الخيط وإعادة إقلاعه. **وGitHub Pages لا يرسل ترويسات مخصّصة بحال**، فهذا الحدّ قائم عليه.

### حدود واجهة الويب

- **البطء**: التنفيذ داخل WebAssembly أبطأ من بايثون الأصلي (٢–٣× تقريبًا). الكتب الكبيرة تُحوَّل أسرع في سطر الأوامر.
- **الذاكرة وحجم الملف**: لسان المتصفّح محدود الذاكرة أكثر من العملية الأصلية. الصفحة تنبّهك على أي ملف يتجاوز **١٥٠ م.ب** (تنبيه لا رفض — الحدّ الحقيقي يعتمد على جهازك)، وإن مات الخيط أثناء التحويل ظهر ذلك في شريط الحالة وعادت الواجهة للعمل بدل أن تتجمّد.
...
- **Scanned files** (no text layer) are read via OCR when Tesseract is installed (`apt install tesseract-ocr tesseract-ocr-ara`, or `brew install tesseract tesseract-lang`); without it they are out of scope and the diagnostic tells you so. The same fallback covers files whose `ToUnicode` map is broken, where Arabic glyphs export as random Latin. Only the pages proven broken are OCR'd; healthy pages keep their original text layer, which is more accurate than any OCR.
- **OCR accuracy is not text-layer accuracy.** Arabic letters come out mostly right, but Arabic-Indic digits are frequently misread, and the recognizer sometimes reads an Arabic word as Latin when `eng` is among the languages — use `--ocr-lang ara` for Arabic-only documents. Always check the numbers in the output. `--ocr never` disables the fallback.
